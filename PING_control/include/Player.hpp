...
#define PHOTO_DIODE_TRESHOLD 3900
...
public:
    Player(){};
    Player(int dir_pin, bool inverted, int step_pin, int end_stop_pin, int solenoid_pin, int photodiod_pin, float max_distance);
    ~Player(){};
    ...
    void release();
    void play();
    void setIsPlaying(bool is_playing){_playing = is_playing;};
    boolean isPlaying(){return _playing;};

    boolean isBallIn();
    void throwIn();//remise en jeu

    boolean isWaittingForThrowIn(){return _waitting_for_throw_in;}
    boolean isBallInStateChange();
...
private:
    ...
    boolean _waitting_for_throw_in = false, _previous_ball_in_state = false, _ready_to_play = false, _playing = false;
    boolean _waiting_for_retirement_ball = false;
