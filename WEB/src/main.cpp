...
#define LIVES 5
int player_lives[4] = {LIVES, LIVES, LIVES, LIVES};
...
void setup()
{
  ...
  server.on("/img/coeur.png", HTTP_GET, [](AsyncWebServerRequest *request)
            { request->send(SPIFFS, "/img/coeur.png", "image/png"); });          
  ...
  server.on("/start", HTTP_GET, [](AsyncWebServerRequest *request)
            {
              ...
              to_send += "1";
              to_send += SEP;
              to_send += String(CODE_BP_THROW_IN);
              to_send += SEP;
              to_send += String(CODE_PUSH);
              Serial.println(to_send); });
  ...
  server.on("/gameStarted", HTTP_GET, [](AsyncWebServerRequest *request)
            { request->send(200, "text/plain", String(gameStarted)+SEP+String(player_lives[0]) + SEP + String(player_lives[1]) + SEP + String(player_lives[2]) + SEP + String(player_lives[3])); });

  server.begin();
}


void receiveGoalsFromSerial()
{
  if (!Serial.available())
    return;
  ...
  player_lives[player_id.toInt() - 1] -= 1; // Le cas vachement rapide et bien

  // // on envoit la liste des vies
  // server.on("/scores", HTTP_GET, [](AsyncWebServerRequest *request)
  //           { request->send(200, "text/plain", String(player_lives[0]) + SEP + String(player_lives[1]) + SEP + String(player_lives[2]) + SEP + String(player_lives[3])); });
}

void loop()
{
  receiveGoalsFromSerial();
}